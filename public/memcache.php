...
define('ADMIN_PASSWORD','silvermachine');       // Admin Password
...
EOB;
    ...
    case 3: // Variables, without slab info
...
EOB;
			...
			foreach ($list as $itemKey => $itemInfo) {
			    $dumpUrl = $PHP_SELF.'&op=2&server='.(array_search($server,$MEMCACHE_SERVERS)).'&dumpslab='.$itemInfo['slabId'];
			    $keyUrl = $PHP_SELF.'&op=4&server='.(array_search($server,$MEMCACHE_SERVERS)).'&key='.base64_encode($itemKey);
				echo
					"<tr class=tr-$m>",
					"<td class=td-0>",'<a href="',$keyUrl,'">',$itemKey,'</a>',"</td>",
					"<td class=td-last>", $itemInfo['info'], "</td></tr>";
				$m=1-$m;
			}
		echo <<<EOB
			</tbody></table>
...
EOB;
        echo "<tr><td class=td-0>",$theserver,"</td><td class=td-0>",$theKey,
             ...
             "</td><td><pre>";
        print_r($r['VALUE'][$theKey]['value']);
        echo "</pre></td>",
             '<td><a href="',$PHP_SELF,'&op=5&server=',(int)$_GET['server'],'&key=',base64_encode($theKey),"\">Delete</a></td>","</tr>";
             
        // the print_r code above used to be: chunk_split($r['VALUE'][$theKey]['value'],40)
        echo <<<EOB
			</tbody></table>
			</div><hr/>
EOB;
